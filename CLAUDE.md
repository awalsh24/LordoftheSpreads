...
- **2026-04-30** — Step 4 public-data portion complete. Connector interface locked: `code/connectors/types.py` (canonical Pydantic models), `code/connectors/base.py` (ExchangeConnector Protocol + exception hierarchy), `code/connectors/hyperliquid.py` (Hyperliquid implementation). Public-data methods (subscribe_book, subscribe_trades, get_mid) fully wired and acceptance-tested against testnet — 5 book updates, 5 trade batches (batch #1 had 30 co-block trades confirming block-batching preserved), get_mid sane. Auth methods stubbed with NotImplementedError pending funded testnet account. Known quirk: `code/` package name shadows stdlib `code` module — entry-point scripts must insert project root at sys.path[0].
- **2026-04-30** — Step 6 prerequisite complete: `code/data/loader.py` unified data loader. Reads live parquet (books + trades), returns canonical OrderBook / list[Trade] objects, reconstructs block-batches by grouping on `time` column. Archive source stubbed with NotImplementedError (format unconfirmed). Acceptance test `scratch/test_loader.py` auto-discovers available dates under data/BTC/, prints 10 trade batches + 5 book snapshots, verifies batch invariants. Run with `python scratch/test_loader.py` from project root.
